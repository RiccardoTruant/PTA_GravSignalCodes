...
my_type S_tot_2(my_type t, const T1 &hc2 ,my_type Sp, const T1 &args,const my_type phi_Pi,const my_type theta_Pi,my_type A_rd,my_type gamma_rd);
...
int main(int argc, char* argv[])
{
   ...
   //frequency grid, gwb, and the gwb with the subtracted contribution if the snr of the source is larger than the setted treshold
	...
	my_type Mc_5_3;
	my_type log10_Mc, z_loc, log10f_gw, q_loc, e_loc,i_loc, PSI_loc, l0_loc,y0_loc,RA_loc, DEC_loc;
    //pol contribution
	my_type a,b;
	my_type MeanAng;   
	...
	{
	    read_model >> log10_Mc  >> z_loc  >> log10f_gw  >> q_loc   >> e_loc   >> i_loc  >> PSI_loc >> l0_loc >> y0_loc >> RA_loc  >> DEC_loc;
...
//Orbital phase
my_type l(my_type t, const T1 &args){
	my_type l0 = args[5];   //intial phase
	my_type f = args[1];    //observed keplerian freq

	return l0 + 2.*pi*f*t;
}

//ANTENNA PATTERN FUNCTIONS:
my_type F_plus(const T1 &args, const my_type phi_Pi, const my_type theta_Pi){
	//Rosado et al 2015, k and j polarization basis vector 
	my_type phi = args[7];   //right ascension GW source RAD
	my_type theta = args[8];  //declination GW sourve RAD
	my_type PSI_GW =args[4];         //Polarization angle
...
my_type F_cross(const T1 &args,const  my_type phi_Pi,const my_type theta_Pi ) 
{
    //Rosado et al 2015, k and j polarization basis vector 
	my_type phi = args[7];   //right ascension GW source RAD
	my_type theta = args[8];  //declination GW sourve RAD
...
my_type Sn_plus(my_type t, int n ,const T1 &args){
	int n1 = n-2, n2= n+2;
    double n_e = n*args[2];
    double jn[5];
    ...
    //Prameters:
	my_type C  = args[0]*G_5_3/c_4; //residual amplitude
	my_type f  = args[1];           //observed keplerian frequency
	my_type w  = 2.*M_PI*f;         //observed keplerian angular velocity
	my_type e  = args[2];           //eccentricity
    my_type i  = args[3];           //inclination angle  
	my_type y0 = args[6];           //pericenter
	
    //functions:
	my_type an =  -C*pow(w,-1./3.) * ( jn[0] - 2.*e*jn[1] + (2./n)*jn[2] + 2.*e*jn[3] - jn[4] ) * sin(n*l(t,args));
	my_type bn =   C*pow(w,-1./3.) * sqrt(1.-e*e)*( jn[0] - 2.*jn[2] + jn[4] ) * cos(n*l(t,args));
	my_type cn =  (2./n)*C*pow(w,-1./3.)*jn[2]*sin(n*l(t,args));

	//function at initial conditions:
	my_type an0 = -C*pow(w,-1./3.) * ( jn[0] - 2.*e*jn[1] + (2./n)*jn[2] + 2.*e*jn[3] - jn[4] ) * sin(n*l(0,args));
	my_type bn0 =  C*pow(w,-1./3.) * sqrt(1.-e*e)*( jn[0] - 2.*jn[2] + jn[4] ) * cos(n*l(0,args));
	my_type cn0 = (2./n)*C*pow(w,-1./3.)*jn[2]*sin(n*l(0,args));
...
my_type Sn_cross(my_type t, int n ,const T1 &args){
	int n1 = n-2, n2= n+2;
    double n_e = n*args[2];
    double jn[5];
    fortran_bessel_jn_(&n1,&n2,&n_e,jn);//between n-2,n+2, argument ne
...
//n residuals:
my_type sn(my_type t, int n ,const T1 &args,const my_type phi_Pi,const my_type theta_Pi){
	return ((F_plus(args,phi_Pi,theta_Pi)*Sn_plus(t,n,args)) + 
		(F_cross(args,phi_Pi,theta_Pi)*Sn_cross(t,n,args))) ;
}
...
my_type S_tot_2(my_type t, const T1 &hc2 ,my_type Sp, const T1 &args,const my_type phi_Pi,const my_type theta_Pi,my_type A_rd,my_type gamma_rd){
	...
	{   
		...
		my_type min_f = 1./(T_obs*year), max_f = 1.e-7; // Hz
	    const int N_bin_f = int( ( max_f-min_f ) / ( 1./(T_obs*year) ) ) + 1;
		...
		my_type Sh_n = ( hc2[int(index)] ) / ( ( f_n*f_n*f_n )*( pi*pi*12. ) );//power spectral density of the SGWB
              
		//red noise
        my_type f_1yr = 1./(60.*60.*24.*365.);
        my_type s_yr = 60.*60.*24.*365.;
		my_type S_red = A_rd*A_rd /(12.*pi*pi) *pow( f_n/f_1yr,-1.*gamma_rd)*s_yr*s_yr*s_yr;
		
		my_type Si_n = Sh_n + Sp + S_red;//total psd gwb + pulsar white noise					
						        
		S += 2.*sn(t,n ,args, phi_Pi, theta_Pi)*sn(t,n,args, phi_Pi, theta_Pi) / Si_n;
	}
...
//Fisher
my_type S_tot_i_j(my_type t,const int i,const int j ,const T1 &hc2 ,my_type Sp, const T1 &args,const my_type phi_Pi,const my_type theta_Pi,my_type A_rd,my_type gamma_rd){
	...
	{   
		...
		//derivative i sn(t,n,vec_1,phi_Pi,theta_Pi)
		T1 vec_i_1_2h = args;
		T1 vec_i_1_1h = args;
		T1 vec_i_2_1h = args;
		T1 vec_i_2_2h = args;

		my_type di = args[i]*1e-6;
		vec_i_1_2h[i] = args[i] + 2.*di;
		vec_i_1_1h[i] = args[i] + 1.*di;
		vec_i_2_1h[i] = args[i] - 1.*di;
		vec_i_2_2h[i] = args[i] - 2.*di;

		//try 5 order accuracy:

		my_type sn_i = (-sn(t,n,vec_i_1_2h,phi_Pi,theta_Pi)+8.*sn(t,n,vec_i_1_1h,phi_Pi,theta_Pi)-8.*sn(t,n,vec_i_2_1h,phi_Pi,theta_Pi) + sn(t,n,vec_i_2_2h,phi_Pi,theta_Pi) ) / (12.*di);
        ...
        my_type dj = args[j]*1e-6;
		vec_j_1_2h[j] = args[j] + 2.*dj;
		vec_j_1_1h[j] = args[j] + 1.*dj;
		vec_j_2_1h[j] = args[j] - 1.*dj;
		vec_j_2_2h[j] = args[j] - 2.*dj;

		my_type sn_j = (-sn(t,n,vec_j_1_2h,phi_Pi,theta_Pi)+8.*sn(t,n,vec_j_1_1h,phi_Pi,theta_Pi)-8.*sn(t,n,vec_j_2_1h,phi_Pi,theta_Pi) + sn(t,n,vec_j_2_2h,phi_Pi,theta_Pi) ) / (12.*dj);
	
						        
		S_i_j += 2.*sn_i*sn_j / Si_n;
	}
